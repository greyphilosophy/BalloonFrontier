...
Each flight can be assigned 1–3 missions selected from the mission pool. Mission selection is deterministic — the same launch configuration always produces the same missions.
...
- **Launch Site**: Recommended launch site
...
Flights are affected by dynamic weather conditions that change based on time of day, altitude, and launch site. Each launch gets deterministic weather conditions based on the configuration.
...
- **Storm Risk** (0.0–0.4): Probability of turbulence during flight
...
```bash
DISCORD_BF_TOKEN="***" python3 start_bot.py
```
