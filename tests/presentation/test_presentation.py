from balloon_frontier.presentation import DiscordAnsiSerializer, FlightPhase, FlightSceneBuilder, PlainTextSerializer, RenderFrame, TerminalAnsiSerializer, build_flight_moments
...
def test_default_timeline_is_seven_frames_and_deterministic():
    first = build_flight_moments(telemetry()); second = build_flight_moments(telemetry())
    assert first == second and len(first) == 7
    assert first[0].phase == FlightPhase.PRELAUNCH and first[-1].phase == FlightPhase.LANDED
    assert sum(m.phase == FlightPhase.APOGEE for m in first) == 1


def test_burst_at_apogee_and_crash_are_preserved():
    moments = build_flight_moments(telemetry(burst=True, landed=False, crashed=True))
...
def test_ansi_serializers_reset_every_line():
    canvas = FlightSceneBuilder().build(RenderFrame(build_flight_moments(telemetry())[3]))
