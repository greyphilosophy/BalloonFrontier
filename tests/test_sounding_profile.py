...
def test_records_regular_layers_and_exact_ceiling():
    ...
    assert [layer.altitude_m for layer in profile.layers] == [
        0.0,
        500.0,
        1000.0,
        1250.0,
...
def test_profile_samples_the_actual_provider():
    ...
    expected = provider.sample(1000.0, time_s=200.0)
...
def test_interpolates_measurement_time_and_ignores_descent():
    ...
    profile = record_sounding_profile(
        (
            _point(0.0, 0.0, 0.0),
            _point(1000.0, 100.0, 10.0),
            _point(500.0, 200.0, -5.0),
        ),
        _weather(),
        TimeVaryingProvider(),
        vertical_resolution_m=500.0,
    ...
    assert [layer.altitude_m for layer in profile.layers] == [0.0, 500.0, 1000.0]
    assert profile.layers[1].wind_x_mps == pytest.approx(50.0)
    assert profile.layers[1].horizontal_velocity_mps == pytest.approx(5.0)
