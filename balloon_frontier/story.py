...
SUMMER_HOBBYIST_CHAPTER = StoryChapter(
    id="summer_hobbyist", title="Summer Project: Edge of Space",
    season="Summer after senior year",
    introduction=("Your last balloon video finally got some views and earned a little revenue. "
    ...
    future_challenges=("Capture Earth and the Moon in the same video.", "Record the longest sunset."),
)
COLLEGE_METEOROLOGY_CHAPTER = StoryChapter(
    id="college_meteorology", title="Atmospheric River", season="Freshman fall",
    introduction=("A meteorology professor has taken an interest in your balloon work. "
        "'We're launching several soundings over the next few days to track an atmospheric river. "
        "Want to do some real work?' she asks, already knowing you cannot refuse."),
    mission_id=ATMOSPHERIC_RIVER_MISSION_ID,
    primary_objective=("Carry weather instruments through the troposphere and recover a vertical "
...
def current_story_chapter(player_id: str | None = None) -> StoryChapter:
    ...
    return COLLEGE_METEOROLOGY_CHAPTER if EDGE_OF_SPACE_MISSION_ID in player.missions_completed else SUMMER_HOBBYIST_CHAPTER
...
def _wind_label(x_mps: float, y_mps: float) -> str:
    ...
    return f"{speed:.1f} {directions[int((bearing + 22.5) // 45.0) % 8]}"
...
def format_atmosphere_profile(profile: AtmosphereProfile, *, max_layers: int = 8) -> str:
    ...
    lines = ["📡 **Recorded atmosphere**", "` Alt km | Temp °C | Press kPa | Wind m/s `"]
    for layer in selected:
        lines.append(
            f"` {layer.altitude_m / 1000:6.1f} | {layer.temperature_k - 273.15:7.1f} |"
            f" {layer.pressure_pa / 1000:9.1f} | {_wind_label(layer.wind_x_mps, layer.wind_y_mps):>8} `"
        )
...
def _stable_footage(outcome: FlightOutcome) -> bool:
    velocities = [point.velocity_mps for point in outcome.result.telemetry if not point.landed]
...
def add_story_results(outcome: FlightOutcome, player_id: str | None = None) -> FlightOutcome:
    ...
    if EDGE_OF_SPACE_MISSION_ID in mission_ids:
        stable, recovered = _stable_footage(outcome), _controlled_recovery(outcome)
        bonuses = (
            MissionResult("bonus_stable_footage", stable, 0,
                "The camera returned steady, watchable footage." if stable else "The footage was too unstable to earn the stable-flight bonus."),
            MissionResult("bonus_controlled_recovery", recovered, 0,
                "The payload completed a controlled recovery." if recovered else "The payload was not recovered under control."),
        )
    ...
    if sounding and sounding.completed and player_id and outcome.weather is not None:
        provider = StandardAtmosphereProvider(site_id=_launch_site_id(outcome), wind_enabled=True)
        profile = profile_from_telemetry(outcome.result.telemetry, outcome.weather, atmosphere_provider=provider)
        atmosphere_profiles.save(str(player_id), profile)
        recorded = MissionResult("bonus_atmosphere_profile", bool(profile.layers), 0,
...
class _LockAtmosphereButton(discord.ui.Button):
    def __init__(self, parent: "StoryConfiguratorMixin") -> None:
        super().__init__(label="Use Recorded Atmosphere", style=discord.ButtonStyle.success,
