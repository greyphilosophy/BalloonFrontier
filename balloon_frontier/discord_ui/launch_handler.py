...
from balloon_frontier.simulation import EnvelopeConfig, SimulationState, run_simulation as run_full_simulation
...
def _ensure_missions_loaded():
    ...
    try:
        mission_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "data", "missions")
        load_mission_directory(mission_dir)
    except Exception:
        logger.exception("Failed to load mission directory")


def run_simulation(gas_type, gas_mass, gas_temperature_k, payload_mass, drag_coeff,
                   envelope_vol, stretch_ratio, envelope_mass_kg=1.0, *,
                   mission_assignment=None, env_config=None, weather_impacts=None,
                   has_pressure_valve=False, launch_altitude_m=0.0,
                   wind_site_id="field"):
    """Backward-compatible fixed-step simulation helper."""
    _ensure_missions_loaded()
    if env_config is None:
        env_config = EnvelopeConfig(max_volume_m3=envelope_vol,
    ...
    state = SimulationState(
        gas_type=gas_type, gas_mass_kg=gas_mass, payload_mass_kg=payload_mass,
        envelope=env_config, altitude_m=launch_altitude_m,
        ...
        wind_enabled=True, wind_site_id=wind_site_id, ballast_mass_kg=0.0,
        has_pressure_valve=has_pressure_valve,
    )
    if mission_assignment:
        max_time = 43200.0
        tel_full = run_full_simulation(state, dt=0.1, total_time_s=max_time,
                                       max_steps=int(max_time / 0.1), step_interval=1.0)
    else:
        tel_full = run_full_simulation(state, dt=0.1, total_time_s=150.0, max_steps=10000)
    if not tel_full:
        return [], {"peak_altitude": 0, "burst": False, "time_of_flight": 0,
                    "payload_count": 1, "score": 0,
                    "medal": medal_tier_to_string(0), "medal_emoji": "⚪"}
    peak_alt = max(t["altitude_m"] for t in tel_full)
    telemetry = [{"time": t["time_s"], "alt": t["altitude_m"],
                  "vel": t["velocity_mps"], "burst": t.get("burst", False),
                  "landed": t.get("landed", False),
                  "crashed": t.get("crashed", False)} for t in tel_full]
    ...
    if mission_assignment:
        summary.update(assigned_missions=list(mission_assignment.get("missions", [])),
...
async def run_launch(configurator: "BalloonConfigurator", interaction: discord.Interaction,
    ...
    try:
        ...
        from balloon_frontier.discord_ui.configurator import GAS_OPTIONS, ENVELOPE_OPTIONS, SITE_OPTIONS, PAYLOAD_OPTIONS
        ...
        launch_request = LaunchRequest(
            gas_id=state["gas"], envelope_id=state["envelope"],
        ...
        payload_display = "None" if list(state.get("payloads") or []) == ["none"] else ", ".join(payload_names)
        chart_str = chart_to_string([r["time"] for r in tel], [r["alt"] for r in tel], title="📈 Flight Trajectory")
        weather = result.weather
        weather_dict = {"name": weather.name if weather else "",
        ...
        assignment_dict = assignment if isinstance(assignment, dict) else {
        ...
        result_content = format_discord_results(
            peak_altitude=result_obj.peak_altitude_m, burst=result_obj.burst,
            landed=result_obj.landed, crashed=result_obj.crashed,
            time_of_flight=result_obj.duration_s, telemetry=tel,
            gas_name=gas_info[0], gas_mass=launch_request.gas_mass_kg,
            env_name=env_info[0], payload_names=payload_display,
            site_name=site_info.name, mission_assignment=assignment_dict,
            player_id=player_id, weather_event=weather_dict, chart_str=chart_str)
        if result.mission_results:
            lines = ["\n🎯 **Mission Results:**"]
            for mr in result.mission_results:
                reward = f" (+{mr.reward} credits)" if mr.reward else ""
                lines.append(f"  {'✅' if mr.completed else '❌'} {mr.mission_id}{reward}: {mr.explanation}")
            mission_text = "\n".join(lines)
