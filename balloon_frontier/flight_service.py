...
from balloon_frontier.aerostat import configured_simulation_state, safety_notes_for_request
from balloon_frontier.atmosphere import AtmosphereProvider, current_atmosphere_provider, use_atmosphere
from balloon_frontier.flight_score import calculate_flight_score
from balloon_frontier.launch_result import FlightResult, LaunchRequest, MissionAssignment, MissionResult, telemetry_list_to_points
...
from balloon_frontier.mission_selection import assign_missions_to_flight, choose_mission_count, seed_from_game_state
...
from balloon_frontier.weather_event import WeatherEvent, generate_weather, weather_impact_on_flight
...
def _state_with_weather_impacts(state: SimulationState, impacts: dict) -> SimulationState:
...
class FlightService:
    ...
    def __init__(
        ...
        self.reward_service = reward_service if reward_service is not None else RewardService(PlayerRegistryRepository())
        self.mission_evaluator = mission_evaluator if mission_evaluator is not None else MissionEvaluator()
    ...
    def prepare(self, launch_request: LaunchRequest) -> LaunchPreparation:
        ...
        weather_seed = seed_from_game_state(
            gas=req.gas_id, envelope=req.envelope_id,
            payloads=payload_keys, site=req.launch_site_id,
        )
        weather = generate_weather(
            site=req.launch_site_id, gas=req.gas_id, envelope=req.envelope_id,
            payloads=payload_keys, seed=weather_seed,
        )
        weather_column = generate_weather_column(weather_seed, scenario=_scenario_for_weather(weather))
        provider = current_atmosphere_provider() or self.atmosphere_provider or weather_column
        with use_atmosphere(provider):
            sim_state = configured_simulation_state(req, req.to_simulation_state())
        ...
        mission_assignment = assign_missions_to_flight(
            payload_count=payload_count, seed=weather_seed, mission_count=mission_count,
            selected_payloads=payload_keys, launch_site=req.launch_site_id,
        )
        weather_impacts = weather_impact_on_flight(weather) if weather else {}
        return LaunchPreparation(
            request=req, sim_state=sim_state, weather=weather,
            mission_assignment=mission_assignment, wind_site_id=wind_site_id,
            weather_impacts=weather_impacts, atmosphere_provider=provider,
        )
    ...
    def _run_prepared(
        ...
        sim_state = (
            ...
            if prep.weather else prep.sim_state
        )
        ...
        powered_result = run_powered_simulation(
            sim_state, payload_ids=tuple(launch_request.payload_ids), dt=0.1,
            total_time_s=max_time, max_steps=max_steps, step_interval=step_interval,
        )
        ...
        if not telemetry:
            return FlightOutcome(
                ...
                weather=prep.weather, mission_assignment=mission_assignment,
                safety_notes=safety_notes, weather_impacts=prep.weather_impacts,
                atmosphere_provider=provider, flight_notes=flight_notes,
            )
        ...
        score = calculate_flight_score(peak_altitude_m, payload_count, duration_s)
        mission_results = self.mission_evaluator.evaluate(
            request=launch_request, telemetry=tuple(points), assignment=mission_assignment,
        )
        mission_results = _mission_results_with_flight_notes(mission_results, flight_notes)
        if launch_request.player_id:
            mission_results = self.reward_service.apply(
                player_id=launch_request.player_id, mission_results=mission_results,
            )

        return FlightOutcome(
            result=FlightResult(telemetry=tuple(points), launch_request=launch_request),
            score=score,
