...
class GraphicFlightSceneRenderer:
    ...
    def _draw_background(self, draw: ImageDraw.ImageDraw, frame: RenderFrame) -> None:
        ...
        if moment.altitude_m >= 12000:
            ...
            self._cloud(
                draw,
                self.width - 104 - frame.cloud_offset_x * 5,
                cloud_y - 30,
                scale=0.7,
            )
        ...
        if (
            moment.phase
            in {FlightPhase.LANDED, FlightPhase.CRASHED, FlightPhase.COMPLETE}
            or moment.altitude_m < 500
        ):
    ...
    def _cloud(
        ...
        *,
        ...
        draw.ellipse(
            (x + int(5 * scale), y, x + w, y + h - int(4 * scale)),
            fill=color,
        )
    ...
    def _draw_vehicle(
        ...
        if moment.phase == FlightPhase.BURST:
            ...
            draw.ellipse(
                (center_x - 19, center_y - 37, center_x - 5, center_y - 10),
                fill=highlight,
            )
            draw.polygon(
                (
            ...
            draw.line(
                (center_x - 7, center_y + 32, center_x - 11, payload_top),
                fill=(56, 57, 64),
                width=1,
            )
            draw.line(
        ...
        if primary in {"quadcopter", "small_quadcopter", "drone"}:
            ...
            self._draw_camera(draw, center_x, payload_top)
        elif primary == "parachute":
            self._draw_parachute_pack(draw, center_x, payload_top)
        else:
        ...
        if len(visible_payloads) > 1:
            ...
            draw.rounded_rectangle(
                (center_x + 20, payload_top - 8, center_x + 43, payload_top + 9),
                radius=6,
                fill=(35, 39, 52),
            )
            draw.text(
                (center_x + 24, payload_top - 6),
                badge,
    ...
    def _draw_burst(draw: ImageDraw.ImageDraw, x: int, y: int) -> None:
        ...
        for polygon in fragments:
            draw.polygon(
                tuple((x + dx, y + dy) for dx, dy in polygon),
                fill=(242, 91, 110),
                outline=(94, 43, 53),
            )
        for dx, dy in ((-43, -27), (42, -22), (-42, 18), (43, 25)):
            draw.line(
                (x + dx // 2, y + dy // 2, x + dx, y + dy),
                fill=(255, 208, 91),
                width=2,
            )

    def _draw_payload_box(
        self,
        draw: ImageDraw.ImageDraw,
        x: int,
        top: int,
    ...
    @staticmethod
    def _draw_camera(draw: ImageDraw.ImageDraw, x: int, top: int) -> None:
        draw.rounded_rectangle(
            (x - 23, top + 3, x + 23, top + 27),
            radius=4,
            fill=(48, 54, 64),
            outline=(21, 24, 30),
            width=2,
        )
        draw.rectangle((x - 13, top - 1, x + 5, top + 5), fill=(64, 70, 81))
        draw.ellipse(
            (x - 10, top + 6, x + 10, top + 26),
            fill=(26, 30, 38),
            outline=(112, 184, 219),
            width=3,
        )
        draw.ellipse((x - 4, top + 12, x + 4, top + 20), fill=(83, 153, 191))

    @staticmethod
    def _draw_parachute_pack(draw: ImageDraw.ImageDraw, x: int, top: int) -> None:
        draw.rounded_rectangle(
            (x - 20, top, x + 20, top + 29),
            radius=8,
            fill=(225, 92, 73),
            outline=(92, 42, 38),
            width=2,
    ...
    def _draw_hud(
        ...
        draw.text(
            (self.width - 18 - (status_bbox[2] - status_bbox[0]), 15),
            status,
            font=self._font,
            fill=self._status_color(moment),
        )

        panel_top = self.height - 38
        draw.rectangle((0, panel_top, self.width, self.height), fill=(11, 15, 25))
        altitude = (
            f"{moment.altitude_m / 1000:.1f} km"
            if moment.altitude_m >= 1000
            else f"{moment.altitude_m:.0f} m"
        )
        draw.text(
            (14, panel_top + 6),
            f"ALT {altitude}",
        ...
        draw.text(
            (141, panel_top + 6),
            f"V/S {moment.velocity_mps:+.1f} m/s",
            font=self._font,
            fill=(121, 219, 235),
        )
        ...
        if frame.event_emphasis and moment.event:
            ...
            draw.text(
                (self.width - 14 - (bbox[2] - bbox[0]), panel_top + 23),
                event,
                font=self._small_font,
                fill=(241, 201, 90),
            )
    ...
    def _status(frame: RenderFrame) -> str:
        ...
        if moment.phase == FlightPhase.LANDED:
            return "RECOVERED"
        if moment.phase == FlightPhase.COMPLETE:
            return "COMPLETE"
        if moment.phase == FlightPhase.BURST:
            return "BURST"
